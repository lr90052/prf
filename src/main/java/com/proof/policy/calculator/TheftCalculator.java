...
@Slf4j
@AllArgsConstructor
public class TheftCalculator implements RiskCalculator {

    final PolicyConfiguration policyConfiguration;

    @Override
    public BigDecimal calculate(final Map<RiskType, BigDecimal> sumInsured2type) {
        BigDecimal insuranceSum = sumInsured2type.getOrDefault(RiskType.THEFT, BigDecimal.ZERO);
        ...
        log.debug("Partial premium result {}: {}", RiskType.THEFT, premiumAddend);
        return premiumAddend;
    }

    private BigDecimal findMatchingCoefficient(BigDecimal value) {
        List<RiskTypeConfig> coefficients = policyConfiguration.getByType(RiskType.THEFT);
        return coefficients.stream()
                ...
                .orElseThrow(() -> new IllegalStateException(
                        "no coefficient config found for " + RiskType.THEFT + ", sum=" + value));
    }
}
